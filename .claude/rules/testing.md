...
- Use `pytest` with no flags — `asyncio_mode = "auto"` and `pythonpath = ["backend"]` are set in `pyproject.toml`.
...
- Always run the full suite after changes: `pytest`. Don't skip unrelated tests.
